...
use Llama\App;
use Llama\Exception\CommandNotFoundException;
use Llama\ServiceInterface;
...
class CommandRegistry implements ServiceInterface
{
    ...
    public function getCommandMap()
    {
        ...
        foreach ($this->namespaces as $command => $namespace) {
            ...
            foreach ($controllers as $subcommand => $controller) {
              $subs[] = [
                "name" => $subcommand,
                "signature" => $namespace->getSignature($subcommand)
              ];
            }
    ...
    public function getSignatureMap()
    {
         ...
         */
        foreach ($this->namespaces as $command => $namespace) {
            $signatures = $namespace->getSignatures();
            foreach ($signatures as $subcommand => $signature) {
                $subs[$subcommand] = $signature;
            }
